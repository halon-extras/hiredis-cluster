...
bool Halon_init(HalonInitContext* hic)
{
	...
	const char* nodes = HalonMTA_config_string_get(HalonMTA_config_object_get(cfg, "nodes"), nullptr);
	const char* password = HalonMTA_config_string_get(HalonMTA_config_object_get(cfg, "password"), nullptr);
	const char* connect_timeout = HalonMTA_config_string_get(HalonMTA_config_object_get(cfg, "connect_timeout"), nullptr);
	const char* timeout = HalonMTA_config_string_get(HalonMTA_config_object_get(cfg, "timeout"), nullptr);
	const char* max_retry = HalonMTA_config_string_get(HalonMTA_config_object_get(cfg, "max_retry"), nullptr);
	...
	for (size_t i = 0; i < pool_size; ++i)
	{
		...
		if (connect_timeout)
		{
			struct timeval tv = { strtol(connect_timeout, nullptr, 10) , 0 };
			redisClusterSetOptionConnectTimeout(cc, tv);
		}
		if (timeout)
		{
			struct timeval tv = { strtol(timeout, nullptr, 10) , 0 };
			redisClusterSetOptionTimeout(cc, tv);
		}
		if (max_retry)
		{
			redisClusterSetOptionMaxRetry(cc, strtol(max_retry, nullptr, 10));
		}
		if (password)
		{
		...
		int ret = redisClusterSetOptionAddNodes(cc, nodes);
		if (ret != REDIS_OK)
		{
			syslog(LOG_CRIT, "hiredis-cluster: bad nodes");
			return false;
		}
